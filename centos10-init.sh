...
# 2. Hostname & Networking (HTB Requirement)
if [ "$(hostname)" != "$TARGET_HOSTNAME" ]; then
    hostnamectl set-hostname "$TARGET_HOSTNAME"
    log_msg "[+] Hostname set to '$TARGET_HOSTNAME'"
    report_msg "Hostname: $TARGET_HOSTNAME"
else
    log_msg "[✓] Hostname already correct"
fi

if ! grep -q "$TARGET_DOMAIN" /etc/hosts; then
    echo "127.0.1.1   $TARGET_DOMAIN $TARGET_HOSTNAME" >> /etc/hosts
    log_msg "[+] Hosts file updated with $TARGET_DOMAIN"
fi
...
# 5. Localization (HTB Requirement: English US)
if [[ $(localectl status) != *"LANG=en_US.UTF-8"* ]]; then
    localectl set-locale LANG=en_US.UTF-8
    log_msg "[+] Locale set to en_US.UTF-8"
fi

# 6. User Creation (Non-Privileged User)
if ! id "$NEW_USER" &>/dev/null; then
    useradd -m -s /bin/bash "$NEW_USER"
    echo "$NEW_USER:$USER_PASS" | chpasswd
    log_msg "[+] User '$NEW_USER' created with provided password"
...
# 7. History Lockdown (HTB Mandatory Security)
...
# Link history for root and the new user
...
report_msg "Security: History files for root and $NEW_USER made immutable"

# 8. Firewall Configuration (Selective Ports)
